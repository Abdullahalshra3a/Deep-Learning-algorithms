...
dataframe = dataframe.loc[(dataframe.Label == "U2R") | (dataframe.Label == "Normal")]
...
newlabel_train=labeltrain.replace({'Normal':0,'U2R':1 })
...
# replace LSTM with SimpleRNN if you want to use RNN

#adding input layer and LSTM layer in one step
model.add(SimpleRNN(128, input_shape=(x_train_3d.shape[1:]), activation='relu', return_sequences=True))
...
plt.title('Validation loss history U2R RNN')
...
plt.title('Validation accuracy history U2R RNN')
